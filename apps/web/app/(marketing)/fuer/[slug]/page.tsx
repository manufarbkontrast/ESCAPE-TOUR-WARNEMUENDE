    ...
    <div className="w-full">
      <section className="container-custom py-16 md:py-24">
        ...
        <div className="mt-8 max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-coast-ink/75">
            {occasion.intro}
          </p>
        </div>

        {/* The numbers this group asks about before anything else. */}
        <div className="mt-10 grid max-w-3xl grid-cols-2 divide-x divide-y divide-white/10 overflow-hidden rounded-xl border border-coast-line sm:grid-cols-4 sm:divide-y-0">
          {occasion.facts.map((fact) => (
            <div key={fact.label} className="px-5 py-4">
              ...
              <div className="mt-1.5 font-mono text-base tabular-nums text-coast-ink">
                {fact.value}
              </div>
            </div>
          ))}
          ...
          <div className="grid gap-x-8 gap-y-10 md:grid-cols-3">
            {occasion.reasons.map((reason) => (
              <div key={reason.title} className="border-t border-white/15 pt-5">
                <h2 className="text-lg font-bold text-coast-ink">{reason.title}</h2>
                <p className="mt-2 text-base leading-relaxed text-coast-muted">{reason.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <section className="border-t border-white/[0.06] py-16 md:py-20">
          <div className="container-custom">
            <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr] lg:gap-16">
              <div>
                <h2 className="font-display text-3xl text-coast-ink md:text-4xl">
                  Unsere Empfehlung für euch
                </h2>
                <p className="mt-4 max-w-xl text-lg leading-relaxed text-coast-ink/65">
                  {occasion.recommendationReason}
                </p>
                <p className="mt-4 max-w-xl text-base leading-relaxed text-coast-muted">
                  Alle drei Varianten laufen dieselbe Route mit denselben zwölf Stationen — sie
                  unterscheiden sich darin, wie hart die Rätsel sind. Wenn eine andere besser passt,
                  sagt Bescheid.
                </p>
              </div>
              ...
              <div className="card">
                ...
                <p className="mt-1 text-sm font-semibold text-coast-muted">{variant.ageLabel}</p>
                ...
                <div className="mt-6 grid grid-cols-3 gap-3 border-y border-coast-line py-4">
                  <div>
                    <div className="font-mono text-[0.65rem] uppercase tracking-[0.18em] text-coast-muted">
                      Dauer
                    </div>
                    <div className="mt-1 font-mono text-sm tabular-nums text-coast-ink">
                      {variant.duration}
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 font-mono text-sm text-coast-ink">
                      {variant.difficulty}
                    </div>
                  </div>
                </div>
        ...
        <div className="container-custom">
          <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
            <div className="max-w-xl">
              ...
              <p className="mt-4 text-lg leading-relaxed text-coast-ink/65">
                Ab etwa zehn Personen stimmen wir Startzeiten und Geräte lieber einmal persönlich
                ab, als dass ihr im Buchungsformular raten müsst. Treffpunkt ist{' '}
                {SITE.meetingPoint.name} {SITE.meetingPoint.detail},{' '}
                {SITE.meetingPoint.minutesBefore} Minuten vor dem Slot.
              </p>
              <a
                href={telHref(SITE.phone.display)}
                className="mt-6 inline-block font-mono text-2xl tabular-nums text-coast-ink transition-colors hover:text-coast-sea"
              >
                {SITE.phone.display}
              </a>
              <div className="mt-1 text-sm text-coast-muted">{SITE.phone.hours}</div>
            </div>
