  ...
  return (
    <div className="space-y-12">
      ...
      <div className={cn('max-w-3xl', isReversed ? 'ml-auto text-right' : '')}>
        <div className="inline-flex items-center gap-2 rounded-full border border-coast-line bg-coast-panel px-4 py-2 text-sm backdrop-blur mb-4">
          <span className="text-coast-ink">
            {tour.difficulty} &middot; {tour.distance} &middot; {tour.duration}
          </span>
        </div>
        <h2 className="text-3xl md:text-4xl font-bold text-coast-ink">{tour.name}</h2>
        <p className="text-coast-ink font-semibold mt-2">{tour.tagline}</p>
        <p className="text-coast-muted mt-4 leading-relaxed">{tour.description}</p>
      </div>

      {/* Routen-Highlights */}
      <div>
        <h3 className="text-xl font-semibold text-coast-ink mb-6">Routen-Highlights</h3>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {tour.highlights.map((highlight, index) => (
            <div key={highlight.name} className="card-hover space-y-3">
              <div className="flex items-center gap-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-coast-ink text-coast-paper font-bold text-sm">
                  {index + 1}
                </div>
                <h4 className="text-lg font-semibold text-coast-ink">{highlight.name}</h4>
              </div>
              <p className="text-sm text-coast-muted font-semibold">{highlight.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card-hover">
        <h3 className="text-xl font-semibold text-coast-ink mb-4">Was euch erwartet</h3>
        <ul className="space-y-3">
          {tour.expectations.map((item) => (
            <li key={item} className="flex items-start gap-3">
              ...
              <span className="text-coast-muted">{item}</span>
            </li>
          ))}
        </ul>

        <div className="mt-8 flex flex-col sm:flex-row gap-4">
          <Link href={`/buchen?variant=${tour.id}`} className="btn btn-primary">
...
function FeatureHighlightCard({ feature }: { readonly feature: FeatureCard }) {
  return (
    <div className="card-hover space-y-4">
      <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-coast-panel border border-coast-line text-coast-ink">
        {feature.icon}
      </div>
      <h3 className="text-xl font-semibold text-coast-ink">{feature.title}</h3>
      <p className="text-coast-muted font-semibold leading-relaxed">{feature.description}</p>
    </div>
  )
}
  ...
  return (
    <div className="w-full">
        ...
        <div className="container-custom relative py-20 md:py-28">
          <div className="mx-auto max-w-3xl text-center space-y-6">
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold">
              Unsere <span className="text-coast-sea">Touren</span>
            </h1>
            <p className="text-lg md:text-xl text-coast-muted max-w-2xl mx-auto">
              Entdeckt Warnemünde mit einer interaktiven GPS-Escape-Tour. Zwei Tourvarianten, ein
      ...
      {TOUR_DETAILS.map((tour, index) => (
        <section
          key={tour.id}
          className={cn('py-16 md:py-24', index % 2 === 0 ? 'bg-coast-paper' : '')}
        >
          ...
          {index < TOUR_DETAILS.length - 1 && (
            <div className="container-custom mt-20">
              <div className="border-t border-coast-line" />
            </div>
          )}
        </section>
      ))}
        ...
        <div className="container-custom">
          <div className="text-center space-y-4 mb-12">
            <h2 className="text-3xl md:text-4xl font-bold">
              Was macht unsere Tour <span className="text-coast-sea">besonders?</span>
            </h2>
            <p className="text-lg text-coast-muted font-semibold max-w-2xl mx-auto">
              Modernste Technologie trifft auf spannende Geschichten. Das unterscheidet uns von
              einer gewöhnlichen Stadtführung.
            </p>
          </div>
      ...
      <section className="py-16 md:py-24 ">
        <div className="container-custom">
          <div className="max-w-3xl mx-auto text-center space-y-8">
            <h2 className="text-3xl md:text-4xl font-bold">
              Bereit für euer <span className="text-coast-sea">Abenteuer?</span>
            </h2>
            <p className="text-lg text-coast-muted">
              Wählt eure Tour und startet wann immer ihr wollt. Die Rätsel des Lotsenkapitäns warten
              auf euch.
            </p>
