...
export function ContactForm() {
    ...
    return (
      <div className="card text-center space-y-4 py-12">
        <div className="flex justify-center">
          ...
          >
            <CheckCircle2 className="h-8 w-8 text-coast-sea" strokeWidth={1.5} />
          </div>
        </div>
        <h3 className="text-2xl font-bold text-coast-ink">Nachricht gesendet!</h3>
        <p className="text-coast-muted font-semibold text-sm max-w-md mx-auto">
  ...
  const inputClasses =
    'w-full rounded-xl px-4 py-3 text-base text-coast-muted placeholder:text-coast-muted focus:outline-none transition-colors'
  const inputStyle = {
    background: '#ffffff',
    border: '1px solid #a8b8b4',
  }
  const inputFocusClass = 'focus:border-coast-line focus:ring-1 focus:ring-white/20'
  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-5 p-6 sm:p-8">
      ...
      {occasion && (
        <div className="rounded-xl border border-neon-400/25 bg-neon-500/[0.05] px-4 py-3">
          <p className="text-sm text-coast-muted">
            Anfrage für{' '}
            <strong className="font-semibold text-coast-ink">{occasion.navLabel}</strong>. Sagt uns
            Personenzahl und Wunschtermin — dann melden wir uns mit einem konkreten Vorschlag
            zurück.
          </p>
        </div>
      )}
      ...
      {status === 'error' && errorMessage && (
        ...
        >
          <AlertCircle className="h-4 w-4 text-red-700/80 flex-shrink-0" strokeWidth={1.5} />
          <p className="text-xs text-red-300">{errorMessage}</p>
        </div>
      )}

      <div>
        <label htmlFor="contact-name" className="block text-sm font-semibold text-coast-muted mb-2">
          Name
        </label>
        <input
          id="contact-name"
      ...
      <div>
        <label
          ...
          className="block text-sm font-semibold text-coast-muted mb-2"
        >
          E-Mail
        </label>
        <input
