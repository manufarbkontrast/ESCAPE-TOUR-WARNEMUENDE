...
function ChevronIcon({ isOpen }: { readonly isOpen: boolean }) {
  return (
    <motion.svg
      className="h-5 w-5 text-coast-ink flex-shrink-0"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
...
function AccordionItem({
  ...
  return (
    <div className="border-b border-coast-line py-4">
      <button
        type="button"
      ...
      >
        <span className="font-sans font-bold text-lg text-coast-ink">{item.question}</span>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="pt-3 pb-1 text-base text-coast-muted font-semibold leading-relaxed">
              {item.answer}
            </p>
          </motion.div>
        )}
